...
from ordonnanceur.SchedulerOPSFlow.assets import excel_dir, get_sigma, get_gamma, generate_valid_orders


def is_valid_order(order, precedence_constraints):
    """
    Vérifie si un ordre respecte les contraintes de précédence.

    :param order: Liste des tâches dans l'ordre (ex: ['T3', 'T1', 'T2', 'T4', 'T5'])
    :param precedence_constraints: Liste de tuples (avant, après) (ex: [('T1', 'T2'), ('T2', 'T3')])
...
def compute_times(order, sigma):
    ...
    for m in range(_num_machines):
        for j, task in enumerate(order):
            if m == 0:
                start = end_times[m][j - 1] if j > 0 else 0
            elif j == 0:
...
gamma = np.array(get_gamma(num_tasks))
task_indices = list(range(num_tasks))

orders = generate_valid_orders(task_indices, gamma)

results = []
for perm in orders: # itertools.permutations(task_indices):
...
wb.save("ordonnancement_permutations0.xlsx")
